...
- `src/lib/components/layout/WorkspaceGrid.svelte`: 4-column desktop layout (rail/explorer/note/code).
...
## Expansion Mapping (Added as Epic 4)
- Graph view:
  - Frontend: `src/lib/features/graph/GraphView.svelte`
  - Backend: `src-tauri/src/core/graph/service.rs`
- Live markdown WYSIWYG toggle:
  - Frontend: `src/lib/features/preview/InlinePreview.svelte`
  - Backend assist: `src-tauri/src/core/links/service.rs` for linked note hints
- Vim mode:
  - Frontend: `src/lib/features/vim/vim-mode.ts`
  - Editor integration: `src/lib/features/editor/CodeEditor.svelte`
- Plugin store + WASM runtime:
  - Frontend: `src/lib/features/plugins/PluginStorePanel.svelte`
