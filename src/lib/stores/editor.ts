...
function createEditorStore() {
	...
	return {
		...
		/** Replace the active file's content buffer, mark dirty, and update tab cache. */
		updateContent(content: string) {
			update((s) => {
				const openFiles = s.openFiles.map((f) =>
					f.path === s.activeFile ? { ...f, content, isDirty: true } : f
				);
				return { ...s, content, isDirty: true, openFiles };
			});
		},
